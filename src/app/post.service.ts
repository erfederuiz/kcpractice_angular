...
export class PostService {
  ...
  getPosts(): Observable<Post[]> {
    ...
    return this._http
      .get(`${this._backendUri}/posts?publicationDate_lte=${moment().valueOf()}&_sort=publicationDate&_order=desc`)
  ...
  getUserPosts(id: number): Observable<Post[]> {
    ...
    return this._http
      .get(`${this._backendUri}/posts?author.id=${id}&publicationDate_lte=${moment().valueOf()}&_sort=publicationDate&_order=desc`)
      .map((response: Response): Post[] => Post.fromJsonToList(response.json()));
  }
  ...
  getCategoryPosts(id: number): Observable<Post[]> {
    ...
    return this._http
      .get(`${this._backendUri}/posts?publicationDate_lte=${moment().valueOf()}&_sort=publicationDate&_order=desc`)
      .map((response: Response): Post[] => Post.fromJsonToList(response.json()))
        .map(posts => posts.filter(p => p.categories.find(c => c.id == id) != null));
  }
  ...
  createPost(post: Post): Observable<Post> {
    ...
    return this._http.post(`${this._backendUri}/posts`, post)
        .map((response: Response): Post => Post.fromJson(response.json()));
  }
